plot_variant_influences_query <- function(data_obj, geno_obj, p_or_q = 0.05,
    ...
    just_main <- var_inf[main_locale,,drop=FALSE]
    ...
    if(length(u_chr) > 0){
        ...
        for(ch in u_chr){
            ...
            target.chr.pos <- target.pos[target.chr.locale]/scale_coord
            source.chr.pos <- source.pos[source.chr.locale]/scale_coord
            ...
            main.chr.pos <- main_pos[main.chr.locale]/scale_coord
            ...
            legend(0,0.8, col = trait_cols[1:length(u_trait)], lty = 1, lwd = 3,
            ...
            mtext(side = 1, paste0("Position (", coord_label, ")"), line = 2.5)
        ...
        plot.text("No interactions found.")
    }
