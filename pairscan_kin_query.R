...
#' 
#' This is a modified version of pairscan_kin that 
#' doesn't require select_markers_for_pairscan to be
#' run. 
#'
...
#' 
#' 
pairscan_kin_query <- function(data_obj, geno_mat, scan_what, marker_pairs, 
kin_obj, verbose = FALSE, run_parallel = FALSE, n_cores = 2){
  ...
  get_marker_pair_stats <- function(m, kin_dat){
    ...
    if(data_obj$kinship_type == "overall"){
      ...
      if(length(non_covar) == 2){kin_name = c(paste(marker_chr, collapse = ","), paste(rev(marker_chr), collapse = ","))}
      
      kin_locale <- which(names(kin_dat) %in% kin_name)
...
#      marker_num <- get_marker_num(data_obj, m)
  ...
  for(p in 1:num_pheno){
    ...
    if(data_obj$kinship_type == "overall"){
        ...
        kin_mat <- kin_obj
        kin_dat <- kin_adjust(kin_mat, geno_mat, chr1 = NULL, chr2 = NULL, 
